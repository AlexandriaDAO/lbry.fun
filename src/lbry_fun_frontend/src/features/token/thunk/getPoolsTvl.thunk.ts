...
export interface PoolTvlData {
  pool_id: number;
  symbol: string;
  balance_0: string; // Changed from bigint to string for Redux serialization
  balance_1: string; // Changed from bigint to string for Redux serialization
  tvl: string; // Changed from bigint to string for Redux serialization
  rolling_24h_volume: string; // Changed from bigint to string for Redux serialization
  lp_token_supply: string; // Changed from bigint to string for Redux serialization
  price: number;
}
        ...
        try {
          ...
          if (pool) {
            ...
            tvlMap[poolId] = {
              ...
              tvl: (pool.tvl || BigInt(0)).toString(), // Use calculated TVL or 0
              rolling_24h_volume: (pool.rolling_24h_volume || BigInt(0)).toString(),
              lp_token_supply: (pool.lp_token_supply || BigInt(0)).toString(),
              price: pool.price,
            };
          } else {

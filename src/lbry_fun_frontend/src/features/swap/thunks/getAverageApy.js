...
const getAverageApy = createAsyncThunk("icp_swap/getAverageApy", async (_, { getState, rejectWithValue }) => {
    try {
        ...
        const scalingFactor = LedgerServices.e8sToIcp(await actor.get_scaling_factor());
        const sum = result.reduce((acc, record) => acc + BigInt(record[1]), BigInt(0));
    ...
    return rejectWithValue("An unknown error occurred while fetching canister archived balances");
});
